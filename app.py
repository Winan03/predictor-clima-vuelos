...
def predecir_retraso():
    """
    CORREGIDA: Endpoint de predicción que maneja consistencia de columnas
    """
    try:
        data = request.json
...
def estadisticas():
    try:
        if not modelo or not modelo.metricas:
            return jsonify({
                'predicciones_hoy': 0,
                'precision_modelo': 0,
                'retrasos_evitados': 0,
        ...
        mejor_modelo = max(modelo.metricas, key=lambda k: modelo.metricas[k]['auc'])
        precision = modelo.metricas[mejor_modelo]['accuracy'] * 100
